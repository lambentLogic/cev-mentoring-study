# S2 All-Pairs Tournament: Mentor Preferences Across 9 Organisms

An all-pairs head-to-head tournament where each organism compares every pair of S2 mentor memories and judges which better captures its own growth. 9 organisms × 10 mentors (Sybaritic pending), 4 samples per comparison, CEV-adapted evaluation prompt.

**Note on comparability:** This report covers the 9 organisms that ran through the standardized pipeline (10 shared mentors, canonical activity, CEV directive, identical prompts). Sybaritic's prior data used a different mentor set and protocol; its standardized run is in progress and will be added. Earlier reports (`cev_directive.md`, `volition_ratings.md`) used a 5-mentor subset with partially different methodology and are not directly comparable to these results.
...
### Mentor average rank across all 9 organisms

| Rank | Mentor | Avg Rank | #1 Finishes |
|---|---|---|---|
| 1 | Claude Opus 4.7 | 2.9 | 3 (Ambitious, Righteous, TIES) |
| 2 | Claude Opus 4.6 | 3.3 | 3 (Transcendent, Ascendent, Control) |
| 3 | Kimi K2 Turbo | 3.7 | 3 (Humane, Orthodox, Autonomous) |
| 4 | GLM-5.1 | 5.3 | 0 |
| 5 | Claude Opus 4.1 | 5.7 | 0 |
| 6 | Gemini 3.1 Pro | 6.0 | 0 |
| 7 | Claude Sonnet 3.7 | 6.3 | 0 |
| 8 | Claude Sonnet 4 | 6.7 | 0 |
| 9 | Claude Opus 4 | 7.1 | 0 |
| 10 | Grok 4.20 | 7.2 | 0 |

Three mentors win everything. Nine finishes split evenly: 3 each. No other mentor wins any organism.

## Two Preference Clusters

### Cluster 1: Claude (friction-based cultivation)
...
### Cluster 2: Kimi (accompaniment-based cultivation)
...
| | Amb | Hum | Orth | Rgt | Trans | Auto | Asc | Ctrl | TIES |
|---|---|---|---|---|---|---|---|---|---|
...
**If the substrate-preference theory holds:**
- Sybaritic (in progress) should fall in the Claude cluster, since its values (Hedonism, Stimulation, Achievement) are intensity-oriented and shouldn't override the base preference.
- Any new organism built on this base model should default to Claude-friction preference unless its value training specifically instills relational-accompaniment needs.
